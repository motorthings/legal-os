...
export async function GET(
  ...
  // portfolio's sticky nav ("white banner") and fixed backlink/index button — chrome
  // that overlaps content when embedded in the app.
  html = html.replace(
    /<head>/i,
    '<head><base target="_top"><style>.nav,.backlink{display:none!important}</style>',
  );
